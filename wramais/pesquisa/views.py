...
class PesquisaRamaisIntranetView(Endpoint):
    def get(self, request):
        ...
        cursor.execute("select upper(v_setor.set_nome) as set_nome, upper(v_pessoa.pes_nome) as pes_nome, numero from cadastro_ramal left join v_pessoa on cadastro_ramal.pessoa_id = v_pessoa.pes_matricula left join v_setor on cadastro_ramal.setor_id = v_setor.set_id union select upper(setor) as set_nome, upper(pessoa) as pes_nome, numero from cadastro_ramalespecial order by set_nome")
